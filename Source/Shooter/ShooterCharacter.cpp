...
AShooterCharacter::AShooterCharacter() :
	...
	CrouchingGroundFritcion(100.f),
	bAimingButtonPressed(false)
{
 	// Set this character to call Tick() every frame.  You can turn this off to improve performance if you don't need it.
	PrimaryActorTick.bCanEverTick = true;

	// Creates a camera boom (pulls in towards character if there is a collision)
	...
	WeaponInterpComp = CreateAbstractDefaultSubobject<USceneComponent>(TEXT("WeaponInterpComp"));
...
void AShooterCharacter::BeginPlay()
{
	...
	// Spawn the default and equip it
...
{
	// Calculate delta for this frame from the rate of information
...
void AShooterCharacter::CameraInterpZoom(float DeltaTime)
{
	...
	if (bAiming)
	{
		// Interping to zoomed FOV
		CameraCurrentFOV = FMath::FInterpTo(CameraCurrentFOV, CameraZoomedFOV, DeltaTime, ZoomInterpSpeed);
	}
	else
	{
		// Interping to default FOV
...
void AShooterCharacter::EquipWeapon(AWeapon* WeaponToEquip)
{
	if (WeaponToEquip)
	{
		...
		if (HandSocket)
		{
			// Attach weapon to the right hand socket
			HandSocket->AttachActor(WeaponToEquip, GetMesh());
		}
		// Set equipped to the newly spawned weapon;
...
void AShooterCharacter::InitialiseAmmoMap()
{
...
void AShooterCharacter::FinishReloading()
{
	...
	{
		// Amount of the ammo the character is carrying of the equipped weapon type
		int32 CarriedAmmo = AmmoMap[AmmoType];

		// Space left in the magazine of equipped weapon
		const int32 MagEmptySpace = EquippedWeapon->GetMagazineCapcity() - EquippedWeapon->GetAmmo();
...
void AShooterCharacter::GrabClip()
{
	...
	int32 ClipBoneIndex{ EquippedWeapon->GetItemMesh()->GetBoneIndex(EquippedWeapon->GetBoneClipName()) };
...
void AShooterCharacter::CrouchButtonPressed()
{
	...
	if (bCrouching)
	{
		...
		GetCharacterMovement()->GroundFriction = CrouchingGroundFritcion;
	}
	else
	{
		GetCharacterMovement()->MaxWalkSpeed = BaseMovementSpeed;
...
void AShooterCharacter::InitialiseInterpLocations()
{
...
void AShooterCharacter::Tick(float DeltaTime)
{
	...
	// Interp the capsule half height based on crouching and standing
...
void AShooterCharacter::IncrementOverllapedItemCount(int8 Amount)
{
...
FVector AShooterCharacter::GetCameraInterpLocation()
{
	...
	return CameraWorldLocation + CameraForward * CameraInterpDistance + FVector(0.f, 0.f, CameraInterpEvelvation);
}
